...
usage(){
	...
	echo "  -n	 Dry run mode. Display the COMMAHND that would have been executed and exit." >&2
	echo "  -s	 Execute the COMMAND using sudo on the remote server." >&2
	echo "  -v	 Verbose mode. Displays the server name vefore executing COMMAND." >&2
	exit 1
}
...
# Make sure the SERVER_LIST file exits.
if [[ ! -e "${SERVER_LIST}"  ]]
then 
